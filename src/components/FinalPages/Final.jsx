    ...
    return <div>
        <div style={{display:"flex"}}>
            ...
            <div className={styles.middle}>
                    <div style={{display:"flex",marginBottom:"1.6rem"}}><div className={styles.campaign}>Campaigns</div><div style={{marginLeft:"7rem",marginTop:"2.8rem"}} ><img src="/sidearrow.png"/></div></div>
                    <div className={styles.allcamp}><div style={{marginTop:"1.6rem",marginLeft:"1.9rem"}}>All Campaigns</div></div>
                    <div className={styles.email}><div  style={{marginTop:"1.6rem",marginLeft:"1.9rem"}}>Email Templates</div></div>
                    <div className={styles.email}><div  style={{marginTop:"1.6rem",marginLeft:"1.9rem"}}>Reports</div></div>
                    <div className={styles.upgrade}><button className={styles.upgradebtn}>Upgrade</button></div>
                    <div style={{marginLeft:"1.7rem"}}><p className={styles.miduser}>{user_name.username}</p><p className={styles.miduserdet}>{campaign}</p></div>
            </div>
            {status?<div className={styles.last}>
                        ...
                        <div className={styles.lastlast}>
                                                   ...
                                                   {!subtit? <p className={styles.who}>What’s the subject line for this campaign?</p>:<></>}
                                                    ...
                                                    {sub?<div style={{display:"flex"}}>
                                                            <div style={{display:"flex",flexDirection:"column",marginRight:"3rem"}}>
                                                                ...
                                                                <div><input type="text" className={styles.subinp} onChange={(e)=>setSubtext(e.target.value)}/></div>
                                                                <div className={styles.firsthint}>See how your<span className={styles.subblue}> recent subject lines</span> performed.<span className={styles.subblue}>View out subject line guide</span></div>
                                                                <div  style={{display:"flex"}} ><div className={styles.subtitle}>Preview Text</div><div className={styles.subchar} style={{marginLeft:"27.4rem"}}>{subtext2.length} characters</div></div>
                                                                <div><input type="text" className={styles.subinp} onChange={(e)=>setSubtext2(e.target.value)}/></div>
                                                                <div className={styles.firsthint}><span className={styles.subblue}>Preview text </span>appears in the inbox after the subject line</div>
                                                                <div  style={{display:"flex",flexDirection:"row"}}>
                                                                    <div><button style={{width:"7.6rem",height:"4rem",border:"1px solid #D9D9D9",borderRadius:"0.2rem",background:!(subtext.length>0&&subtext2.length>0)?"transparent":"#716B67",color:!(subtext.length>0&&subtext2.length>0)?"#D9D9D9":"#FFFFFF"}} className={styles.subsavebtn} onClick={()=>togglesubgt()}>Save</button></div>
                                                                    <div style={{marginTop:"1.2rem"}} className={styles.subcancelbtn} onClick={()=>setSub(false)}>Cancel</div>
                                                                </div>
                                                            </div>
                                                            <div style={{width:"44rem",marginBottom:"3rem",marginTop:"-0.5rem"}}>
                                                                <img style={{width:"100%"}} src={!(subtext.length>0&&subtext2.length>0)?"/imgguide1.png":'/imgprovider3.png'}/>
                                                            </div>
                                                    </div>:<div></div>}
                                                </div>
                                                <div>
                                    ...
                                    <div className={styles.llfo}>
                                        <div  style={{width:"2.4rem",height:"2.4rem",marginTop:"2.4rem",marginLeft:"2rem"}}><img src="/whitetick.png"/></div>
                                        <div  style={{display:"flex",flexDirection:"column",marginLeft:"1rem"}}>
